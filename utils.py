...
from transformers import BertTokenizer, BertModel, AutoFeatureExtractor, AutoModel
import numpy as np
import random
import torch


def narration_generator(inputs):
    narration_begining_list = ['the person ', 'the actor ', 'the guy ']
    # narration_conection_point = [', next ', ', then ', ', after that ', ', in the next step ']
    final_narrations = []
    for input_narration in inputs:
        final_narrations.append(random.choice(narration_begining_list) + input_narration + '.')
...
class text_feature_generator():
    def __init__(self, model_name='bert-base-uncased'):
        ...
        self.model = BertModel.from_pretrained(model_name).cuda()
        self.tokenizer = BertTokenizer.from_pretrained(model_name)
        self.model.eval()

    def creating_feature_vector(self, sentences, seq_len):
        # inputs = self.tokenizer(sentence, return_tensors="pt")
        feature_vectors = torch.zeros(len(sentences[0]), seq_len, 768)
        with torch.no_grad():
            for index in range(len(sentences[0])):
                for action_index in range(seq_len):
                    tokens = self.tokenizer(sentences[action_index][index], padding=True, truncation=True, return_tensors="pt")
                    tokens = {k: v.cuda() for k, v in tokens.items()}
                    #import pdb; pdb.set_trace()
                    outputs = self.model(**tokens)
                    feature_vector = outputs.last_hidden_state[:, 0, :]
                    feature_vectors[index][action_index] = feature_vector[0]
        return feature_vectors

def visual_featuer_extractor(model_name):
