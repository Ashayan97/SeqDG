...
def mixup_data_pipeline_grl(x_v, x_t, t_x_v, y, alpha=1.0, dataset ='epic'):
    '''Returns mixed inputs, pairs of targets, and lambda'''
    if alpha > 0:
        lam = np.random.beta(alpha, alpha)
    else:
    ...
    mixed_t_x_v = lam * t_x_v + (1 - lam) * t_x_v[index, :]
    if dataset == 'epic':
        y_a = {}
        y_b = {}
        # y_a = torch.tensor(y_a).to('cuda')
        ...
        y_a, y_b = y, y[index]
    return mixed_x_v, mixed_x_t, mixed_t_x_v, y_a, y_b, lam



def mixup_data_pipeline(x_v, x_t, y, alpha=1.0, dataset ='epic'):
    '''Returns mixed inputs, pairs of targets, and lambda'''
    if alpha > 0:
        lam = np.random.beta(alpha, alpha)
    else:
    ...
    if dataset == 'epic':
        ...
        y_a, y_b = y, y[index]
    return mixed_x_v, mixed_x_t, y_a, y_b, lam
