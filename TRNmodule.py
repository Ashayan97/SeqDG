...
class RelationModule(torch.nn.Module):
    ...
    def forward(self, input):
        input = input.view(input.size(0), input.size(1), self.num_frames*self.img_feature_dim)
...
class RelationModuleMultiScale(torch.nn.Module):
    ...
    def __init__(self, img_feature_dim, num_bottleneck, num_frames, clip_number):
    ...
    def forward(self, input):
        ...
        act_scale_1 = act_scale_1.view(act_scale_1.size(0), self.clip_number, self.scales[0] * self.img_feature_dim)
        act_scale_1 = self.fc_fusion_scales[0](act_scale_1)
        act_scale_1 = act_scale_1.unsqueeze(2) # add one dimension for the later concatenation
        ...
        for scaleID in range(1, len(self.scales)):
            ...
            for idx in idx_relations_evensample:
                ...
                act_relation = act_relation.view(act_relation.size(0), self.clip_number, self.scales[scaleID] * self.img_feature_dim)
                act_relation = self.fc_fusion_scales[scaleID](act_relation)
                act_relation = act_relation.unsqueeze(2)  # add one dimension for the later concatenation
                act_relation_all += act_relation

            act_all = torch.cat((act_all, act_relation_all), 2)
        return act_all
